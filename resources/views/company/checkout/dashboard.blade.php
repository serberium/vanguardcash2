<x-app-layout>
        ...
        <div class="mb-8 border-b border-gray-200">
            <nav class="flex space-x-8">
                ...
                <a href="{{ route('checkout.status') }}"
                    ...
                    {{ !isset($caixa) || !$caixa ? 'Abrir Caixa' : 'Fechar Caixa' }}
                </a>
    ...
    <div class="py-12 max-w-4xl mx-auto">
        <div class="bg-white p-6 rounded shadow">
            ...
            @if(isset($caixa) && $caixa && isset($resumo) && $resumo)
            <h2 class="text-2xl font-bold mb-6">Resumo do Caixa - {{ ucfirst($caixa->status) }}</h2>

            <div class="grid grid-cols-2 gap-4">
                <div class="p-4 bg-gray-100 rounded">Saldo Inicial: <strong>R$ {{ number_format($resumo['abertura'] ??
                        0, 2, ',', '.') }}</strong></div>
                ...
                <div class="p-4 bg-green-100 rounded">Vendas Dinheiro: <strong>R$ {{ number_format($resumo['dinheiro']
                        ?? 0, 2, ',', '.') }}</strong></div>
                <div class="p-4 bg-blue-100 rounded">Vendas PIX: <strong>R$ {{ number_format($resumo['pix'] ?? 0, 2,
                        ',', '.') }}</strong></div>
                <div class="p-4 bg-purple-100 rounded">Vendas Cartão: <strong>R$ {{ number_format($resumo['cartao'] ??
                        0, 2, ',', '.') }}</strong></div>
            ...
            @php
            $totalVendas = ($resumo['dinheiro'] ?? 0) + ($resumo['pix'] ?? 0) + ($resumo['cartao'] ?? 0);
            $totalSangrias = abs($resumo['sangria'] ?? 0);
            $saldoEsperado = (($resumo['abertura'] ?? 0) + $totalVendas) - $totalSangrias;
            $diferenca = ($caixa->closing_amount ?? 0) - $saldoEsperado;
            @endphp
            ...
            <div class="mt-8 p-6 bg-gray-50 rounded border-t-4 border-gray-800">
                ...
                <div class="space-y-2">
                    <p>Saldo Esperado: <strong>R$ {{ number_format($saldoEsperado, 2, ',', '.') }}</strong></p>
                    <p>Valor Declarado: <strong>R$ {{ number_format($caixa->closing_amount ?? 0, 2, ',', '.')
                            }}</strong></p>
            ...
            <div class="text-center py-10">
                <h2 class="text-2xl font-bold text-gray-700">Nenhum caixa aberto</h2>
                <p class="text-gray-500 mt-2">Você não possui um caixa ativo no momento.</p>
                <a href="{{ route('checkout.status') }}"
                    class="mt-4 inline-block bg-blue-600 text-white px-6 py-2 rounded">Ir para Abertura de Caixa</a>
            </div>
            @endif
        </div>
    </div>
